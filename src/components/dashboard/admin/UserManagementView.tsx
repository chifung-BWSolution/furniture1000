...
import { UserCog, UserPlus, Check, X, Power, Mail, ChevronDown, Loader2, RefreshCw } from 'lucide-react';
import {
  ROLE_META, PERMISSIONS, ROLE_PERMISSIONS,
  type PlatformUser, type UserRole,
} from '@/constants/analytics-mock';
import {
  fetchPlatformAdminData, updatePlatformUserProfile, invitePlatformUser,
} from '@/lib/adminApi';
...
function fmt(d: string) {
  const x = new Date(d);
  if (Number.isNaN(x.getTime()) || x.getTime() <= 0) return '—';
...
export function UserManagementView() {
  ...
  const toggleActive = async (id: string) => {
    ...
    const res = await updatePlatformUserProfile(id, { active: next });
    if (res.ok) {
      toast.success(next ? '已啟用帳號' : '已停用帳號', { description: u.name });
    } else {
      setUsers((prev) => prev.map((x) => (x.id === id ? { ...x, active: u.active } : x)));
  ...
  const changeRole = async (id: string, role: UserRole) => {
    ...
    setUsers((prevUsers) => prevUsers.map((u) => (u.id === id ? { ...u, role } : u)));
    const res = await updatePlatformUserProfile(id, { role });
    if (res.ok) {
      toast.success('已更新角色', { description: ROLE_META[role].label });
    } else {
      if (prev) {
        setUsers((prevUsers) => prevUsers.map((u) => (u.id === id ? { ...u, role: prev } : u)));
      }
      toast.error('更新失敗', { description: res.error });
    }
  };
  ...
  const invite = async () => {
    ...
    const res = await invitePlatformUser(email, inviteRole);
    if (res.ok) {
      toast.success('已新增用戶', { description: email });
      setShowInvite(false);
      setInviteEmail('');
      await load(true);
    } else {
      toast.error('邀請失敗', { description: res.error });
    }
  };
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border bg-muted/30 px-6 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono-data text-[11px] text-muted-foreground">
            {users.length} 位用戶 · {users.filter((u) => u.active).length} 啟用中
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
          ...
          >
            <RefreshCw className={cn('h-3.5 w-3.5', refreshing && 'animate-spin')} /> 同步
          </button>
          <button
            type="button"
            onClick={() => setShowInvite(true)}
            className="flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm hover:opacity-90"
          >
            <UserPlus className="h-3.5 w-3.5" /> 邀請新用戶
          </button>
        </div>
      </div>
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            {users.length === 0 ? (
              <div className="px-6 py-16 text-center">
                <UserCog className="mx-auto h-8 w-8 text-muted-foreground/40" />
                <p className="mt-3 font-display text-sm text-muted-foreground">尚無用戶資料</p>
                <p className="mt-1 text-[12px] text-muted-foreground/70">
                  請確認 PMS 用戶已同步，或點擊「同步」重新載入
                </p>
              </div>
            ) : (
                    ...
                    <tr key={u.id} className="hover:bg-muted/30">
                      <td className="px-5 py-2.5">
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 font-display text-[12px] font-bold text-primary">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="relative inline-block">
                          <select
                            ...
                            className={cn('appearance-none rounded-full border px-2.5 py-0.5 pr-6 text-[10.5px] font-medium focus:outline-none', ROLE_META[u.role].className)}
                          >
                            {ROLE_OPTIONS.map((r) => (
                      ...
                      <td className="px-3 py-2.5 text-right">
                        <button
                          ...
                          className={cn('inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[11px]', u.active ? 'border-border text-rose-500 hover:bg-rose-500/10' : 'border-border text-emerald-600 hover:bg-emerald-500/10')}
                        >
              ...
              <div className="flex items-center gap-1 rounded-lg border border-border bg-background p-0.5">
                {ROLE_OPTIONS.map((r) => (
                  <button
                    ...
                    className={cn('rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors', permRole === r ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground')}
                  >
                    {ROLE_META[r].label}
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowInvite(false)}>
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setShowInvite(false)} className="rounded p-1 text-muted-foreground hover:bg-muted"><X className="h-4 w-4" /></button>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-border bg-background pl-8 pr-3 py-2 font-body text-[13px] focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20"
                  />
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ROLE_OPTIONS.map((r) => (
                    <button
                      ...
                      className={cn('rounded-full border px-3 py-1 text-[11.5px] font-medium transition-colors', inviteRole === r ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-muted-foreground hover:text-foreground')}
                    >
                      {ROLE_META[r].label}
                    </button>
                  ))}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setShowInvite(false)} className="rounded-lg border border-border px-3.5 py-2 text-xs font-medium text-muted-foreground hover:bg-accent">取消</button>
              <button type="button" onClick={() => void invite()} className="flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground hover:opacity-90">
                <UserPlus className="h-3.5 w-3.5" /> 新增用戶
              </button>
            </div>
          </div>
        </div>
