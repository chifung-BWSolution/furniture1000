 ...
 *                   Includes SKU, options, and metafields for every product.
 */
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    do {
      page++;
      // Include options in fields so we get product-level option names
      let url = `https://${shopDomain}/admin/api/2024-10/products.json?limit=250&status=any&fields=id,title,body_html,vendor,product_type,handle,status,published_at,images,variants,tags,options,created_at,updated_at`;
    ...
    if (!body.import) {
      const preview = allProducts.map((p: Record<string, unknown>) => {
        ...
        // Extract primary SKU from first variant
        const primarySku = variants.length > 0 ? (variants[0].sku || "") : "";
        return {
    ...
    // ── Fetch metafields for each product in parallel batches ───────────
    // Shopify REST: GET /admin/api/2024-10/products/{id}/metafields.json
    const METAFIELD_CONCURRENCY = 5;
    const metafieldsMap: Record<string, any[]> = {};

    for (let i = 0; i < toImport.length; i += METAFIELD_CONCURRENCY) {
      const batch = toImport.slice(i, i + METAFIELD_CONCURRENCY);
      const results = await Promise.allSettled(
        batch.map(async (p: Record<string, unknown>) => {
          const pid = String(p.id);
          let allMeta: any[] = [];
          let metaPageInfo: string | null = null;

          do {
            let metaUrl = `https://${shopDomain}/admin/api/2024-10/products/${pid}/metafields.json?limit=250`;
            if (metaPageInfo) metaUrl += `&page_info=${metaPageInfo}`;
            ...
            if (!metaResp.ok) {
              console.warn(`[fetch-shopify-products] Metafields fetch failed for product ${pid}: ${metaResp.status}`);
              break;
            }

            const metaData = await metaResp.json();
            allMeta.push(...(metaData.metafields ?? []));

            const metaLinkHeader = metaResp.headers.get("Link") ?? "";
            const metaNextMatch = metaLinkHeader.match(/<[^>]*[?&]page_info=([^>&"]+)[^>]*>;\s*rel="next"/);
            metaPageInfo = metaNextMatch ? metaNextMatch[1] : null;
          } while (metaPageInfo);

          return { pid, metafields: allMeta };
        })
      );

      for (const result of results) {
        if (result.status === "fulfilled") {
          metafieldsMap[result.value.pid] = result.value.metafields;
        }
      }
    }

    console.log(`[fetch-shopify-products] Fetched metafields for ${Object.keys(metafieldsMap).length} products`);
    ...
    const rows = toImport.map((p: Record<string, unknown>) => {
      ...
      // Primary SKU: from first variant
      const primarySku = variants.length > 0 ? (variants[0].sku || "") : "";

      // Normalize metafields: store as {namespace, key, value, type}[]
      const rawMetafields = metafieldsMap[pid] ?? [];
      const normalizedMetafields = rawMetafields.map((m: any) => ({
        id: m.id,
      ...
      // Normalize variants: include all fields including barcode, weight, fulfillment_service
      ...
      // Normalize options: [{name, position, values[]}]
      const normalizedOptions = ((p.options as any[]) ?? []).map((o: any) => ({
        id: o.id,
        name: o.name,
        position: o.position,
        values: o.values ?? [],
      ...
      return {
        ...
        tags,
        sku: primarySku,
        metafields: normalizedMetafields.length > 0 ? normalizedMetafields : null,
        price: minPrice || null,
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      ...
      if (upsertErr) {
        ...
        metafieldsCount += batch.filter((r) => r.metafields && (r.metafields as any[]).length > 0).length;
      }
    }

    return json({
      success: true,
      message: `已導入 ${imported} 件產品至 shopify_products（含 metafields: ${metafieldsCount} 件）`,
      imported,
      total_selected: toImport.length,
      products_with_metafields: metafieldsCount,
      shop_domain: shopDomain,
    });
