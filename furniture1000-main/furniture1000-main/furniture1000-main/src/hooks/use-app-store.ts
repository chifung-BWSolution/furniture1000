...
export function useAppStore() {
  ...
  const reloadProducts = useCallback(async () => {
    try {
      ...
      // Only update products if the DB returned data; avoid wiping the local state
      // when a transient auth/network issue returns an empty result set.
      if (loaded.length > 0 || (productRows !== null && productRows !== undefined)) {
        setProducts(loaded);
