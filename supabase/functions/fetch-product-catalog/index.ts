...
type CatalogFilters = {
  ...
  level1?: string;
  level2?: string;
  /** When level1 is set without level2, filter master.category to these level2 names */
  level2_categories?: string[];
};

function applyCatalogFilters<T extends { ilike: Function; eq: Function; in: Function }>(
  query: T,
  filters: CatalogFilters,
): T {
  const { search = "", factory_name = "", level2 = "", level2_categories = [] } = filters;

  if (search.trim()) {
    query = query.ilike("title", `%${search.trim()}%`) as T;
  }

  if (factory_name.trim()) {
    query = query.eq("factory_name", factory_name.trim()) as T;
  }

  if (level2.trim()) {
    query = query.eq("category", level2.trim()) as T;
  } else if (level2_categories.length > 0) {
    query = query.in("category", level2_categories) as T;
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const filters: CatalogFilters = {
      ...
      level1,
      level2,
      level2_categories: Array.isArray(level2_categories) ? level2_categories : [],
    };
    ...
    if (error) {
      ...
      let fallbackQuery = applyCatalogFilters(
        masterSupabase
          .from("bwf_product_master")
      ...
      if (fbError) {
        ...
        let minimalQuery = applyCatalogFilters(
          masterSupabase
            .from("bwf_product_master")
