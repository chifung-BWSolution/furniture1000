-- Persist editable SKU on quote line items (editor-only; not shown on customer PDF).

ALTER TABLE public.bwf_quote_item
  ADD COLUMN IF NOT EXISTS sku TEXT NULL;

COMMENT ON COLUMN public.bwf_quote_item.sku IS
  'Internal / catalog SKU for quote draft editing; excluded from customer PDF.';
