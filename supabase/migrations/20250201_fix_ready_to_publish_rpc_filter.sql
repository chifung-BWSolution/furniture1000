-- 準備上載 must include furniture_group_checked IS NULL OR true (see 20250172).
-- A3 RPCs incorrectly filtered only = true, hiding ~355 legacy rows.

create index if not exists ready_to_shopify_ready_publish_v2_idx
  on public.ready_to_shopify (ready_to_publish_at desc nulls last, imported_at desc)
  where furniture_group_checked is distinct from false;
  ...
  where p.shopify_product_id is null
    ...
    and (
      ...
      or (p_stage = 'ready-to-publish'
        and r.furniture_group_checked is distinct from false)
    )
  order by 1;
$$;
...
as $$
  ...
  from public.ready_to_shopify r
  join public.products p on p.id = r.product_id
  where r.furniture_group_checked is distinct from false
    and (
