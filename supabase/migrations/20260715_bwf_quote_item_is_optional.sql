-- Add optional-product flag to quote line items (reference-only; excluded from totals).

ALTER TABLE public.bwf_quote_item
  ADD COLUMN IF NOT EXISTS is_optional BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN public.bwf_quote_item.is_optional IS
  'When true, line is reference-only: HKD subtotal is 0 and excluded from quote totals; PDF shows 可選產品 + checkbox.';
...
-- Legacy project_data.items extractor: include is_optional when present.
