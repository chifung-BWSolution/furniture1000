-- Allow freeform dimension text on quote lines (e.g. 1000+600 / 1000-600).
-- Keep column names; store as text so PDF/editor can show input as-is.

ALTER TABLE public.bwf_quote_item
  ALTER COLUMN dimension_l_mm TYPE text
  USING CASE
    WHEN dimension_l_mm IS NULL THEN NULL
    WHEN dimension_l_mm = trunc(dimension_l_mm)
      THEN trunc(dimension_l_mm)::bigint::text
    ELSE rtrim(rtrim(dimension_l_mm::text, '0'), '.')
  END;

ALTER TABLE public.bwf_quote_item
  ALTER COLUMN dimension_w_mm TYPE text
  USING CASE
    WHEN dimension_w_mm IS NULL THEN NULL
    WHEN dimension_w_mm = trunc(dimension_w_mm)
      THEN trunc(dimension_w_mm)::bigint::text
    ELSE rtrim(rtrim(dimension_w_mm::text, '0'), '.')
  END;

ALTER TABLE public.bwf_quote_item
  ALTER COLUMN dimension_h_mm TYPE text
  USING CASE
    WHEN dimension_h_mm IS NULL THEN NULL
    WHEN dimension_h_mm = trunc(dimension_h_mm)
      THEN trunc(dimension_h_mm)::bigint::text
    ELSE rtrim(rtrim(dimension_h_mm::text, '0'), '.')
  END;

COMMENT ON COLUMN public.bwf_quote_item.dimension_l_mm IS
  'Length/diameter (mm) as freeform text — may include expressions like 1000+600.';
COMMENT ON COLUMN public.bwf_quote_item.dimension_w_mm IS
  'Width (mm) as freeform text.';
COMMENT ON COLUMN public.bwf_quote_item.dimension_h_mm IS
  'Height (mm) as freeform text.';
