-- Hide quote line items from customer PDF / preview while keeping them in the draft editor.

ALTER TABLE public.bwf_quote_item
  ADD COLUMN IF NOT EXISTS hide_in_pdf BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN public.bwf_quote_item.hide_in_pdf IS
  'When true, the line stays in the quote draft editor but is omitted from PDF / Quotation Preview.';
