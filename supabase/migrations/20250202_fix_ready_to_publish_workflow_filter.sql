-- 準備上載 = passed 傢俬組檢查 (furniture_group_checked true), or legacy null+info_done.
-- Must NOT include info_done=false rows (those belong in 產品信息 / 產品文案).
  ...
  where p.shopify_product_id is null
    ...
    and (
      ...
      or (p_stage = 'ready-to-publish'
        and (
          r.furniture_group_checked = true
          or (r.furniture_group_checked is null and r.info_done = true)
        ))
    )
  order by 1;
$$;
